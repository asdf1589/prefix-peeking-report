...
\title{Prefix Peeking: Eviction Policy and Scheduling Method for LLM Serving with Long Shared Contexts}
...
We present two complementary, opt-in mechanisms implemented in the vLLM v1
engine and compatible with the \jenga~\cite{jenga2025} SOSP'25 heterogeneous
KV cache allocator (the mechanisms operate at the scheduling layer and are
orthogonal to allocator placement):
...
\textbf{Practical visibility of the waiting queue.}
In a single-node LLM serving process, the scheduler and the KV cache manager
share the same address space and the same in-process data structures.
The lifecycle of a request is as follows: (1)~a client request arrives at
the HTTP front-end and is appended to the waiting queue; (2)~the scheduler
reads the waiting queue at each engine step to select the next batch;
(3)~the KV cache allocator assigns blocks and launches the prefill kernel;
(4)~autoregressive decoding proceeds; and (5)~the completed response is
returned.
Steps~(1)--(2) reveal that the waiting queue is already a live, in-process
object that the scheduler reads unconditionally at every step.
Both \ppe\ and \pps\ merely \emph{reuse} this already-visible queue to
inform eviction and ordering decisions; they introduce no inter-process
communication, no additional memory copies, and no external lookups.
This design is therefore deployable in any production single-node vLLM
instance without architectural changes.
...
Both mechanisms are implemented in the vLLM v1 engine.  They operate at the
scheduling layer and are orthogonal to the KV cache allocator, so they
compose with \jenga~\cite{jenga2025}, a SOSP'25 system for heterogeneous KV
cache management, as well as with the stock vLLM allocator used in our
evaluation.
...
Algorithm~\ref{alg:ppe} gives the pseudocode.

\begin{algorithm}[t]
\caption{Prefix Peeking Eviction (\ppe)}
\label{alg:ppe}
\begin{algorithmic}[1]
\Require Waiting queue $Q$; free block list $F$; total block count $N_{\mathrm{total}}$
\State $B_{\mathrm{avg}} \gets \mathrm{mean\,blocks}(\text{first } \min(5, |Q|) \text{ requests in } Q)$
\State $\rho \gets N_{\mathrm{total}} / B_{\mathrm{avg}}$
\If{$\rho < \rhomin$} \textbf{return} \Comment{auto-disabled; fall back to pure LRU}
\EndIf
\State $\mathrm{budget} \gets \max(1, \lfloor |F| / 2 \rfloor)$
\State $P \gets \emptyset$ \Comment{protected set}
\For{each request $r$ in $Q$}
  \For{each free block $b$ reachable from $r$'s prefix hash chain}
    \If{$|P| < \mathrm{budget}$}
      \State $P \gets P \cup \{b\}$
    \EndIf
  \EndFor
\EndFor
\State \textbf{LRU eviction loop:} skip any block $b \in P$
\end{algorithmic}
\end{algorithm}
...
times.
The standard LRU eviction loop walks the free list until it collects
the required number of evictable blocks; protected blocks are skipped.
Because at most half the free blocks may be protected, the loop inspects at
most $2k$ entries to evict $k$ blocks in the worst case, so the amortised
number of entries scanned per evicted block is bounded by a constant
($\leq 2$) independent of the protected-set size.
This worst-case bound holds regardless of how protected blocks are positioned
within the list, and keeps the amortised eviction cost $O(1)$.
...
\pps\ replaces the default FCFS request waiting queue with a priority queue that
...
The sort is triggered only when a new request joins
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figure1_throughput.pdf}
  \caption{Closed-loop throughput across models and workloads.
           ...
           The large throughput gap between (b) and (a) reflects context-length
           scaling: MMLU-pro prompts average $\approx$4k tokens while arXiv QA
           prompts average 90k tokens. Under flash attention, prefill time
           scales approximately linearly with sequence length, so the
           22$\times$ longer arXiv inputs account for a comparable reduction
           in throughput; cache-pressure overhead (evictions, preemptions, and
           re-prefills under LRU) adds a further multiplier, yielding the
           observed $\approx$100$\times$ gap for Gemma-3-4B-IT and
           $\approx$215$\times$ for Llama-3.1-8B.}
  \label{fig:bar}
\end{figure*}
...
\subsection{Prefix Cache Hit Rate}
\label{sec:hitrate}

Figure~\ref{fig:hitrate} shows the prefix cache hit rate reported by the vLLM
logger (10-second rolling average) over the course of the closed-loop
Gemma-3-4B-IT arXiv QA run for all four policies.
All four conditions run on the stock vLLM base used for the LRU baseline in
Table~\ref{tab:gemma}, and differ only in whether \ppe\ and \pps\ are active.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figure3_hit_rate.pdf}
  \caption{Prefix cache hit rate --- Gemma-3-4B-IT, arXiv QA
           (90k context), closed-loop, all four policies.  Each point is the
           10\,s rolling average reported by the vLLM engine logger.
           All four conditions run on the stock vLLM base, matching the LRU
           baseline definition used in Table~\ref{tab:gemma}.}
  \label{fig:hitrate}
\end{figure}

\textbf{LRU.}
Under stock vLLM with no extra cache tiers, the GPU KV cache fills quickly
with 88\,k-token contexts and LRU evicts warm blocks whenever pressure
rises.  The hit rate stays low throughout: mean 2.5\%, ranging 0--5.2\%,
with no sustained upward trend.

\textbf{\ppe-only.}
\ppe\ raises the mean to 3.1\% (range 0--9.3\%).  The improvement is modest
in absolute terms because cache pressure under stock vLLM is high enough
that protection alone cannot retain warm blocks across many eviction cycles.
The pattern confirms that \ppe\ is a \emph{retention} mechanism: once warm
blocks survive a scheduling round they stay available, but the constrained
pool limits how many can be protected simultaneously.

\textbf{\pps-only.}
\pps\ produces a higher peak hit rate than \ppe, reaching 11.6\% while LRU
peaks at 5.2\%.  The mean is 3.7\%, also above LRU.
By scheduling requests with the longest estimated prefix overlap first, \pps\
front-loads hits onto the requests that benefit most from the warm cache
window before pressure evicts those blocks.
\pps\ acts as a \emph{prioritisation} mechanism: it directs requests to
cached content as soon as any cache content exists.

\textbf{Both.}
Combining both mechanisms yields the highest sustained hit rate (mean
4.7\%, max 8.6\%).  \pps\ accelerates early hits and \ppe\ extends how long
those warm blocks survive, so the two mechanisms are complementary even
under tight cache budgets.

\textbf{Hit rate is a contributing, not sole, mediator.}
The aggregate hit-rate ordering (LRU\,$<$\,\ppe\,$<$\,\pps\,$<$\,Both) is
consistent with the throughput ordering in Table~\ref{tab:gemma}, but the
mapping is non-linear: \ppe's \mbox{+0.6\,pp} mean improvement over LRU
produces no reliable throughput gain (within run-to-run SD), while \pps's
\mbox{+1.2\,pp} improvement yields $+37\%$ throughput.
This suggests that \emph{when} hits occur matters as much as \emph{how
many}: \pps\ front-loads hits onto the highest-reuse requests, converting
cache warmth into actual prefill savings; \ppe\ raises the aggregate count
but without this load-ordering effect the marginal throughput gain requires
a larger cache pool (i.e., higher $\rho$) to accumulate.
Beyond raw hit rate, both mechanisms also reduce \emph{preemption and
recomputation churn}; isolating each mediator's contribution quantitatively
is left for future work.  Figure~\ref{fig:hitrate} is therefore
\emph{consistent with} but does not fully explain the throughput
results in Table~\ref{tab:gemma}.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figure2_rate_sweep.pdf}
...
\textbf{Operator deployment guidance.}
An operator can estimate $\rho$ before enabling \ppe\ without running the
workload.  Given a model, a target context length $L$ tokens, a KV block size
$T$ tokens, and the GPU KV pool size $N_{\mathrm{total}}$ blocks (reported by
vLLM at startup), the per-article block cost is
$B_{\mathrm{avg}}\approx\lceil L/T\rceil$ and
$\rho \approx N_{\mathrm{total}} / B_{\mathrm{avg}}$ (Equation~\ref{eq:rho}).
Enable \ppe\ when $\rho\geq\rhomin$; otherwise the auto-disable path makes it a
no-op, so leaving it on is harmless.
The anti-starvation threshold $\tau$ should be set near the deployment's TTFT
SLO: the 5\,s default suits batch/offline serving, whereas interactive
deployments with sub-second TTFT targets should reduce $\tau$ accordingly so
that \pps\ never defers a request past its latency budget.
Finally, because \pps\ prioritises requests that share cached prefixes, it can
favour tenants whose traffic happens to overlap; in multi-tenant settings the
anti-starvation band bounds the worst-case delay, but per-tenant fairness
weighting is a natural extension we leave to future work.
...
\textbf{Hit rate measured for Gemma-3-4B-IT only.}
Section~\ref{sec:hitrate} reports prefix cache hit rate for all four policies
on Gemma-3-4B-IT.  Extending this measurement to Ministral-8B FP8 and
Llama-3.1-8B would strengthen the mechanistic link across all three
pool-size regimes.
...
The general idea of cache-aware scheduling is therefore not new; \pps's
specific contribution is the combination of (i)~a \emph{read-only} prefix-hit
estimator that reuses the allocator's existing lookup path with no state
mutation, (ii)~a single-band \emph{anti-starvation} guarantee folded into the
sort key rather than a separate ageing pass, and (iii)~an \emph{enqueue-time
 re-sort} that keeps per-step dequeue $O(1)$.  These three properties make
\pps\ a drop-in, side-effect-free addition to an existing single-node engine,
which prior multi-node or clustering-based designs do not provide.
...
\pps\ prioritises cache-warm requests via a priority sort with an
